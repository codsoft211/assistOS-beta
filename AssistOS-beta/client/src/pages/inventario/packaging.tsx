...
import { Box, Search, Eye, Edit, ShoppingCart, ArrowUpDown } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function PackagingPage() {
  ...
  const packaging = products.filter((p) => p.itemType === 'PACKAGING');

  const filteredProducts = packaging.filter((product) => {
    return !searchQuery || 
    ...
    return (
      <div className="p-6 space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Containers, boxes, and disposables</p>
        </div>
        <Card>
  ...
  return (
    <div className="p-6 space-y-6" data-testid="page-packaging">
      <div className="flex flex-row items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold" data-testid="text-page-title">
            <Box className="inline h-8 w-8 mr-2 text-gray-600" />
            Consumables
          </h1>
          <p className="text-muted-foreground">
            {filteredProducts.length} consumable items
          </p>
        </div>
