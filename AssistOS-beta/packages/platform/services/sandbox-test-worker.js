      ...
      if (/\.zip\//.test(binPath)) {
        ...
        if (pnpapi) {
          ...
          const binTargetPath = path2.join(
            ...
            ".cache",
            "esbuild",
            `pnpapi-${pkg.replace("/", "-")}-${"0.25.9"}-${path2.basename(subpath)}`
          );
...
// packages/platform/services/sandbox-test-worker.ts
...
async function compileTypeScript(tsCode, filePath) {
  if (!esbuild) {
    return null;
  }
  try {
...
function executeJavaScript(jsCode, filePath) {
  try {
    const moduleExports = {};
...
async function executeTests() {
  const { tempDir, files } = import_worker_threads.workerData;
  const results = [];
  try {
    ...
    for (const [filePath, content] of Object.entries(files)) {
      ...
      if (filePath.endsWith(".js")) {
        ...
        if (execResult.error) {
          try {
            const module2 = require(fullPath);
            results.push({
              name: `JavaScript execution: ${filePath}`,
              passed: true,
              output: `Successfully loaded module with ${Object.keys(module2).length} export(s)`
            });
            if (Object.keys(module2).length > 0) {
              results.push({
                name: `Export verification: ${filePath}`,
                passed: true,
                output: `Exports: ${Object.keys(module2).join(", ")}`
              });
            }
          } catch (requireError) {
...
executeTests().then((output) => {
  if (import_worker_threads.parentPort) {
    import_worker_threads.parentPort.postMessage(output);
  } else {
