...
// Source: /tmp/assistos-legacy/server/ai-tools.ts (5776 lines)

// TODO: storage service needs to be migrated
// import { storage } from "../../../apps/api/services/storage.service";

// TODO: onboarding-storage service needs to be migrated
// import { getTenantStorage } from "../../../apps/api/services/onboarding-storage";

// NOTE: These types don't exist in clean schema - commented out for now
// TODO: Add these types to schema if needed
...
import { users, type UserPreferences } from "../../../shared/schema";
import { and, eq, gte, lte, desc, ilike, or, sql } from "drizzle-orm";
import { execSync } from "child_process";
import * as path from "path";

// NOTE: ERP services disabled - not needed for Assist Start onboarding
...
import { connectorTools, executeConnectorTool } from "./connector-tools";
import { schemaEvolutionTools, executeSchemaEvolutionTool } from "./schema-evolution-tools";
import { tocOnlineTools, executeTOCOnlineTool } from "./toc-online-tools";
...
NUNCA envies credenciais vazias! Sempre pede ao utilizador se faltarem dados.`,
  ...
  // === USER PREFERENCES TOOLS (for Settings Agent) ===
  {
    type: "function" as const,
    function: {
      name: "get_user_preferences",
      description: "Obtém as preferências pessoais atuais do utilizador (tema, módulos escondidos, notificações, etc.)",
      parameters: {
        type: "object",
        properties: {},
      },
  ...
  },
  {
    ...
    function: {
      ...
      description: "Atualiza as preferências pessoais do utilizador. Pode atualizar parcialmente (apenas os campos fornecidos serão modificados).",
      parameters: {
        type: "object",
        properties: {
          preferences: {
            type: "object",
            description: "Objeto com as preferências a atualizar (partial update)",
            properties: {
              autoAlerts: {
                type: "boolean",
                description: "Alertas automáticos de emails importantes",
              },
              emailNotifications: {
                type: "boolean",
              ...
              desktopNotifications: {
                type: "boolean",
                description: "Notificações desktop",
              },
              hiddenModules: {
                type: "array",
                items: { type: "string" },
                description: "Array de slugs dos módulos a esconder na sidebar (ex: ['inventory', 'production'])",
              },
              sidebarCollapsed: {
                type: "boolean",
                description: "Sidebar colapsada por padrão",
              },
              favoriteModules: {
                type: "array",
                items: { type: "string" },
                description: "Array de slugs dos módulos favoritos",
              },
              theme: {
                type: "string",
              ...
              defaultView: {
                ...
                description: "Vista padrão para entidades",
              },
              itemsPerPage: {
                type: "number",
              ...
              chatSoundEnabled: {
                type: "boolean",
                description: "Som ativado nos chats",
              },
              showTypingIndicator: {
                type: "boolean",
                description: "Mostrar indicador de digitação nos chats",
              },
            },
          },
        },
  ...
  // Bash Tools - Performance Optimization (delegate heavy work to scripts)
  {
    type: "function" as const,
    function: {
      name: "check_platform_state",
      description: "Verifica o estado da plataforma (módulos ativos, disponibilidade de dados). Use SEMPRE antes de oferecer funcionalidades ao utilizador.",
      parameters: {
        type: "object",
        properties: {
          tenantId: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Valida ficheiros CSV/Excel para importação. Retorna erros específicos por linha.",
      parameters: {
        type: "object",
        properties: {
          filePath: {
            type: "string",
            description: "Caminho completo do ficheiro",
          },
          importType: {
            type: "string",
            enum: ["clients", "products"],
  ...
  },
  {
    type: "function" as const,
    function: {
      name: "search_brand_files",
      description: "Procura informação nos ficheiros da marca (catálogos, descrições de produtos). Use quando precisar de contexto sobre produtos da empresa.",
      parameters: {
        type: "object",
        properties: {
          tenantId: {
  ...
  },
  {
    ...
    function: {
      name: "test_erp_connection",
      description: "Testa conectividade com um sistema ERP. Retorna status HTTP e tempo de resposta.",
      parameters: {
        type: "object",
        properties: {
          connectionId: {
  ...
  },
  {
    ...
    function: {
      ...
      description: "Normaliza dados da empresa (sector, tipo) para valores standard.",
      parameters: {
        type: "object",
        properties: {
          sector: {
            type: "string",
            description: "Sector da empresa (ex: 'tecnologia', 'construção')",
          },
          type: {
            type: "string",
            description: "Tipo de negócio (ex: 'B2B', 'B2C')",
          },
        },
        required: ["sector", "type"],
      },
    },
  },
  ...
  // Connector Configuration Tools (for AssistBuild)
  ...connectorTools,
  
  // Schema Evolution Tools (for AssistBuild - Gap #3)
  ...schemaEvolutionTools,
];
...
export interface AIToolContext {
  ...
  userId?: string;
  sessionId?: string;
  environment?: string; // Critical: sandbox, development, production
}
...
export async function executeAITool(
  ...
  const environment = typeof context === 'object' ? context?.environment : undefined;
  try {
    switch (toolName) {
...
- Sempre avisa: "✅ Tarefa criada! Vê na página Tarefas" quando crias uma tarefa`;
      ...
      case "get_user_preferences": {
        if (!userId) {
          return { 
            success: false, 
            error: "User ID não fornecido. Preferências são pessoais do utilizador." 
          };
        }
        ...
        try {
          ...
          console.error("[get_user_preferences] Error:", error);
      ...
      case "update_user_preferences": {
        if (!userId) {
          return { 
            success: false, 
            error: "User ID não fornecido. Só podes modificar as TUAS preferências." 
          };
        }
        ...
        try {
          ...
          const updatedFields = Object.keys(preferences);
          let message = "✅ Preferências atualizadas com sucesso!\\n\\n";
          
          if (preferences.hiddenModules !== undefined) {
            message += `📁 Módulos escondidos: ${preferences.hiddenModules.length === 0 ? "Nenhum (todos visíveis)" : preferences.hiddenModules.join(", ")}\\n`;
          }
          if (preferences.theme) {
            message += `🎨 Tema: ${preferences.theme}\\n`;
          }
          if (preferences.language) {
            message += `🌍 Idioma: ${preferences.language}\\n`;
          }
          if (preferences.autoAlerts !== undefined) {
            message += `🔔 Alertas automáticos: ${preferences.autoAlerts ? "Ativados" : "Desativados"}\\n`;
          }
          ...
          console.error("[update_user_preferences] Error:", error);
          return { success: false, error: error.message };
        }
      }

      // === BASH TOOLS (Performance Optimization) ===
      case "check_platform_state": {
        const { tenantId: targetTenantId } = args;
        ...
        try {
          const scriptPath = path.join(process.cwd(), "server", "tools", "check-platform-state.sh");
          const result = execSync(`bash "${scriptPath}" "${finalTenantId}"`, {
            encoding: "utf-8",
            env: process.env,
            timeout: 5000,
          });
          ...
          console.error("[check_platform_state] Error:", error);
          return { 
            success: false, 
            error: error.stderr || error.message || "Erro ao verificar estado da plataforma" 
          };
        }
      }

      case "validate_import_file": {
        const { filePath, importType } = args;
        
        if (!filePath) {
          return { success: false, error: "Caminho do ficheiro é necessário" };
        }

        try {
          const scriptPath = path.join(process.cwd(), "server", "tools", "validate-import-file.sh");
          const result = execSync(`bash "${scriptPath}" "${filePath}" "${importType || 'clients'}"`, {
            encoding: "utf-8",
            timeout: 10000,
          });
          ...
          return { 
            success: false, 
            errors: [error.message || "Erro ao validar ficheiro"] 
          };
        }
      }

      case "search_brand_files": {
        const { tenantId: targetTenantId, query } = args;
        const finalTenantId = targetTenantId || tenantId;
        
        if (!finalTenantId || !query) {
          return { success: false, error: "TenantId e query são necessários" };
        }

        try {
          const scriptPath = path.join(process.cwd(), "server", "tools", "search-brand-files.sh");
          const result = execSync(`bash "${scriptPath}" "${finalTenantId}" "${query}"`, {
            encoding: "utf-8",
            timeout: 8000,
          });
          ...
          console.error("[search_brand_files] Error:", error);
          return { 
            success: false, 
            error: error.message || "Erro ao procurar ficheiros da marca" 
          };
        }
      }
      ...
      case "test_erp_connection": {
        ...
        if (!connectionId) {
          return { success: false, error: "ConnectionId é necessário" };
        }

        try {
          const scriptPath = path.join(process.cwd(), "server", "tools", "test-erp-connection.sh");
          const result = execSync(`bash "${scriptPath}" "${connectionId}"`, {
            encoding: "utf-8",
            env: process.env,
            timeout: 15000,
          });
          ...
          console.error("[test_erp_connection] Error:", error);
          ...
          return { 
            success: false, 
            error: error.message || "Erro ao testar conexão ERP" 
          };
        }
      }

      case "normalize_company_data": {
        const { sector, type } = args;
        
        if (!sector || !type) {
          return { success: false, error: "Sector e type são necessários" };
        }

        try {
          const input = JSON.stringify({ sector, type });
          const scriptPath = path.join(process.cwd(), "server", "tools", "normalize-company-data.sh");
          const result = execSync(`bash "${scriptPath}" '${input}'`, {
            encoding: "utf-8",
            timeout: 3000,
          });
          ...
          console.error("[normalize_company_data] Error:", error);
          return { 
            success: false, 
            error: error.message || "Erro ao normalizar dados da empresa" 
          };
        }
      }

      // ============ CONNECTOR CONFIGURATION TOOLS (AssistBuild) ============
      ...
      case "get_field_mappings": {
        if (!tenantId || !userId) {
          return {
            success: false,
            error: "tenantId and userId are required for connector operations"
          };
        }
        
        // CRITICAL: Pass environment from context to ensure correct tenant isolation
        return await executeConnectorTool(toolName, args, { tenantId, userId, environment });
      }

      // ============ SCHEMA EVOLUTION TOOLS (AssistBuild - Gap #3) ============
      ...
      case "list_pending_migrations": {
        if (!tenantId || !userId) {
          return {
            success: false,
            error: "tenantId and userId are required for schema evolution operations"
          };
        }
        
        // CRITICAL: Pass environment from context to ensure correct tenant isolation
        return await executeSchemaEvolutionTool(toolName, args, { tenantId, userId, environment });
      }

      // ============ TOC ONLINE ACTION TOOLS (AssistME) ============
      ...
      case "list_toc_sync_pending": {
        if (!tenantId || !userId) {
          return {
            success: false,
            error: "tenantId and userId are required for TOC Online operations"
          };
        }
        
        // CRITICAL: Pass environment from context to ensure correct tenant isolation
        return await executeTOCOnlineTool(toolName, args, { tenantId, userId, environment });
      }
