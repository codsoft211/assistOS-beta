...
import { evaluateCondition } from '../../../../packages/execution/conditionEvaluator.js';
...
export class ExecutionEngine {
  ...
  async execute(context: ExecutionContext): Promise<void> {
    ...
    try {
      ...
      for (const node of executionOrder) {
        ...
        const incomingEdges = workflow.definition.edges.filter((e: any) => e.target === node.id);

        if (incomingEdges.length > 0) {
          // A node executes if AT LEAST ONE incoming edge condition is true
          // If edge has no condition, it's considered true
          const shouldExecute = incomingEdges.some((edge: any) => {
          ...
          if (!shouldExecute) {
            logger.info(
              { workflowId, executionId, nodeId: node.id },
              '[ExecutionEngine] Skipping node due to condition'
            );
            await this.logSkippedNode(node, context);
            continue;
          }
        }
        // -------------------------------
   ...
   * Log skipped node
   */
  private async logSkippedNode(node: AssistBuildNode, context: ExecutionContext): Promise<void> {
